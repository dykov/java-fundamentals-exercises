...
public class CrazyStreams {
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
        return accounts.stream()
                .collect(Collectors.groupingBy(account -> Sex.MALE.equals(account.getSex())));
    }
    ...
    public Map<String, List<Account>> groupAccountsByEmailDomain() {
        return accounts.stream()
                .collect(Collectors.groupingBy(this::getEmailDomain));
    }

    private String getEmailDomain(final Account account) {
        final int domainIndex = account.getEmail().lastIndexOf('@') + 1;
        return account.getEmail().substring(domainIndex);
    }
